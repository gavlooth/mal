...
pub fn init_grammar() Grammar {
    ...
    const grammar_rules = " number  \"number\"  : /[0-9]+/ ; symbol  \"symbol\"  : /[a-zA-Z0-9_+\\-*\\/\\\\=<>!&]+/ ;  string  \"string\"  : /\"(\\\\.|[^\"])*\"/ ;comment : /;[^\\r\\n]*/ ;sexpr : '(' <expr>* ')' ;qexpr: '{' <expr>* '}' ;expr : <number>  | <symbol> | <string>| <comment> | <sexpr>  | <qexpr> ;lispy : /^/ <expr>* /$/ ; ";
...
pub fn parse_input_string(input: []u8, lispy: ?*mpc.mpc_parser_t) ![*c]mpc_ast_t {
    var r: mpc.mpc_result_t = undefined;
    const result = mpc.mpc_parse("input", @as([*c]u8, @ptrCast(@alignCast(input))), lispy, &r) != 0;
    if (result) {
        const the_ast: [*c]mpc_ast_t = @as([*c]mpc_ast_t, @ptrCast(@alignCast(r.output)));
        return the_ast;
    }
    return error.UnparsableString;
}
